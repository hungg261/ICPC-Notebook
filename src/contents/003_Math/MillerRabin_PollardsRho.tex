...
Deterministic Miller-Rabin primality test for 64-bit integers in $O(K \log^3 N)$ alongside Pollard's Rho algorithm for prime factorization in $O(N^{1/4})$.
\begin{lstlisting}
...
Use \lstinline|miller_rabin(n)| to check primality. To find all prime factors, use \lstinline|factorize(n, factors)| then \lstinline|sort(factors.begin(), factors.end())|.
